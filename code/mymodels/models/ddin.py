...
class DDIN(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, target_feature_list, pv_history_fc_names=None, click_history_fc_names=None
                , dnn_use_bn=False,
                 dnn_hidden_units=(512, 256), dnn_activation='relu', att_hidden_size=(64, 16),
                 att_activation='Dice', att_weight_normalization=False, l2_reg_dnn=0.0,
                 l2_reg_embedding=1e-6, dnn_dropout=0.5, init_std=0.0001,
        ...
        self.pv_gate = nn.Sequential(
            nn.Linear(self.d_model, self.d_model, bias=False),  # 稍微增加神经元数量
            nn.LeakyReLU(),
            #nn.Dropout(0.3),  # 维持 Dropout
            nn.Linear(self.d_model, self.d_model, bias=False),  # 保持这一层的神经元数量不变
            nn.Sigmoid()
        )
        self.click_gate = nn.Sequential(
            nn.Linear(self.d_model, self.d_model, bias=False),  # 稍微增加神经元数量
            nn.LeakyReLU(),
            nn.Dropout(),  # 维持 Dropout
            nn.Linear(self.d_model, self.d_model, bias=False),  # 逐层递减
                                         # 保持这一层的神经元数量不变
            nn.Sigmoid()
        )
    ...
    def forward(self, X):
        ...
        # pv seq 去噪
        #pv_keys_emb = self.pv_gate(pv_keys_emb)

        pv_keys_emb_gate = self.pv_gate(pv_keys_emb)  # 这才是原始的去噪逻辑
        pv_keys_emb = pv_keys_emb * pv_keys_emb_gate
        ...
        pv_hist = self.pv_attention(query_emb, pv_keys_emb, pv_keys_length)           # [B, 1, E]
        ...
        # click seq 去噪
        click_keys_emb_gate = self.click_gate(click_keys_emb)  # 这才是原始的去噪逻辑
        click_keys_emb = click_keys_emb * click_keys_emb_gate
        ...
        click_hist = self.click_attention(query_emb, click_keys_emb, click_keys_length)           # [B, 1, E]
