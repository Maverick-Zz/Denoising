...
class DMR(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, target_feature_list, history_fc_names=None, dnn_use_bn=False,
        ...
        super(DMR, self).__init__([], dnn_feature_columns, l2_reg_linear=0, l2_reg_embedding=l2_reg_embedding,
    ...
    def item_to_item(self, query_emb, keys_emb, keys_length):
        """ 计算 query 与 keys_emb 中每个元素的相似度 """
        
        similarity_score = torch.cosine_similarity(query_emb, keys_emb, dim=-1) # [batchsize, seq_len]
        similarity_score = similarity_score * keys_length
    ...
    def forward(self, X):
        ...
        item_to_item_score = self.item_to_item(self, query_emb, keys_emb, keys_length)

        """ user to item network """
        user_vec = torch.sum(keys_emb, dim=1, keepdim=False)
        user_to_item_score = torch.cosine_similarity(user_vec, query_emb, dim=-1)

        # deep part
        deep_input_emb = torch.cat((deep_input_emb, hist, item_to_item_score, user_to_item_score), dim=-1)
