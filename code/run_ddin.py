...
fixlen_feature_columns = [SparseFeat(feat, data[feat].nunique(), embedding_dim=17)
                          for feat in sparse_features]


# 创建了一个包含了序列特征的 VarLenSparseFeat 对象的列表。
# VarLenSparseFeat 是 DeepCTR 库中定义的一个特征类，用于处理不定长的序列特征。
varlen_feature_columns = [VarLenSparseFeat(SparseFeat('pv_seq', vocabulary_size=len(pv_seq_key2index) + 1, embedding_dim=17),maxlen=pv_seq_max_len, length_name="pv_seq_length"),
                          VarLenSparseFeat(SparseFeat('click_seq', vocabulary_size=len(click_seq_key2index) + 1, embedding_dim=17),maxlen=click_seq_max_len, length_name="click_seq_length")]
...
# dnn_dropout=0.5, init_std=0.0028  auc = 0.8158
# split_date = 893216638
...
model = DDIN(dnn_feature_columns, target_feature_list=["item_id"], pv_history_fc_names=["pv_seq"], click_history_fc_names=["click_seq"],
            task='binary',
