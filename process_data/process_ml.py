...
#
# #给dateframe列名重命名
# user_column_names = {'user_id:token': 'user_id', 'age:token': 'age', 'gender:token': 'gender', 'occupation:token': 'occupation', 'zip_code:token': 'zip_code'}
# df_user.rename(columns=user_column_names, inplace=True)
# item_column_names = {'item_id:token': 'item_id', 'movie_title:token_seq': 'movie_title', 'release_year:token': 'release_year', 'class:token_seq': 'class'}
# df_item.rename(columns=item_column_names, inplace=True)
# inter_column_names = {'user_id:token': 'user_id','item_id:token': 'item_id', 'rating:float': 'rating', 'timestamp:float': 'timestamp'}
...
#
#
# df_inter, inter_label_encoders = preprocess_dataframe(df_inter)
...
#
# # 在循环中，对于每个唯一的用户ID，代码从 df1 中选择该用户的所有数据，然后按照 time_ms 列进行降序排序
#
# def get_behavior(user_data, timestamp, topk=50, day=1):
#     # 从用户的行为数据中获取行为序列，并进行一定的数据增强
#     """
...
#     """
#
#     user_data = user_data[user_data['timestamp'] < timestamp]  # pv
#     # 筛选出了时间戳小于当前时间戳的用户行为数据，用于构建行为序列。
#
#     # pv
#     pv_data = user_data[user_data['rating'] <= 3]  # 全部 pv
#     pv_seq = list(pv_data['item_id'])
#     # 分别筛选出了不同类型的用户行为数据，并将视频 ID 存储在相应的列表中。
#
#     # click
#     click_data = user_data[user_data['rating'] > 3]  # 全部 click
#     click_seq = list(click_data['item_id'])
#
#     pv_seq = pv_seq[: topk]
#     click_seq = click_seq[: topk]
#
#     # 组成序列
#     pv_seq_str = ""
#     click_seq_str = ""
#
#     if len(pv_seq) >= 1:
#         pv_seq = list(map(lambda x: str(x), pv_seq))
#         pv_seq_str = ",".join(pv_seq)
#         # 将 PV 行为序列转换为字符串
#     if len(click_seq) >= 1:
#         click_seq = list(map(lambda x: str(x), click_seq))
#         click_seq_str = ",".join(click_seq)
#
#     seq_dict = {}
#     seq_dict['pv_seq'] = pv_seq_str
#     seq_dict['click_seq'] = click_seq_str
#
#     return seq_dict
#
#
...
#     seq_dict = get_behavior(user_data, timestamp)  # 获得了 seq
#     data['pv_seq'] = seq_dict['pv_seq']
#     data['click_seq'] = seq_dict['click_seq']
#     return data
#
#
#
# data = df1.copy()
# #data = df1.head(10000).copy()
# # data = data.parallel_apply(append_behavior_to_sample, axis=1)
...
#
#
# save_to_pickle(data, "../data/ML_Pure/processed/test.pkl")
#
# #print(data)
